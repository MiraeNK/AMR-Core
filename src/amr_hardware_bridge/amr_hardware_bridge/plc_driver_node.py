...
class PlcDriverNode(Node):
    ...
    def _load_parameters(self):
        ...
        p.zero_crossing_s    = float(

            self.get_parameter('zero_crossing_cooldown_s').value)
        ...
        self.get_logger().info(
            ...
            'zc_cooldown={}s  jump_filter={}m'.format(

                p.wheel_diameter_m, p.wheel_separation_m,

                p.ticks_per_rev, p.speed_min, p.speed_max,

                p.zero_crossing_s, p.jump_filter_m,
    ...
    def _cmd_callback(self, msg):
        ...
        if len(msg.data) < 2:
            ...
            self.get_logger().warn(
                ...
                '[ CMD ] /wheel_cmd_vel butuh 2 elemen [right, left]. '
        ...
        with self._lock:
            ...
            self._cmd_right = int(
                ...
                max(p.speed_min, min(p.speed_max, msg.data[0])))

            self._cmd_left  = int(

                max(p.speed_min, min(p.speed_max, msg.data[1])))
    ...
    def _try_connect(self):
        ...
        try:
            ...
            self.get_logger().error(
                ...
                'Retry dalam {}s.'.format(exc, self._p.reconnect_s)
    ...
    def _try_reconnect_if_needed(self, now):
        ...
        if not self._plc_connected and now >= self._next_reconnect:
            ...
            self.get_logger().info('[ PLC ] Mencoba reconnect...')
    ...
    # ZERO-CROSSING INTERLOCK

    # =========================================================================



    def _apply_zero_crossing(self, target_right, target_left, now):

        """Harus dipanggil di dalam blok with self._lock."""
        ...
        def crosses_zero(prev, tgt):

            return prev != 0 and ((prev > 0) != (tgt > 0))



        r_cross = crosses_zero(self._prev_right, target_right)

        l_cross = crosses_zero(self._prev_left, target_left)



        if r_cross or l_cross:

            self._locked_until = now + self._p.zero_crossing_s
            ...
            self.get_logger().info(
                ...
                '[ ZC ] Zero-crossing -> cooldown {}s'.format(

                    self._p.zero_crossing_s)
    ...
    def _cycle_callback(self):
        ...
        with self._lock:
            ...
            out_r, out_l = self._apply_zero_crossing(

                self._cmd_right, self._cmd_left, now)
            ...
            if self._last_left_ticks is not None:
                ...
                if abs(dl_m) >= p.jump_filter_m or abs(dr_m) >= p.jump_filter_m:
                    ...
                    self.get_logger().warn(
                        ...
                        '-> siklus diabaikan.'.format(dl_m, dr_m)
    ...
    def destroy_node(self):
        ...
        self.get_logger().info(
            ...
            '[ plc_driver_node ] Shutdown — nol-kan motor...')
